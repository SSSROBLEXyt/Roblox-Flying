...
-- GUI creation (kept visually similar to your original)
...
-- Close, minimize and maximize buttons should be children of the Frame (not main.Frame which doesn't exist)
...
-- Notification (kept from original but you can remove or change)
StarterGui:SetCore("SendNotification", {
    Title = "Adnan Tik Engine s.m.w.r.o";
    Text = "Adnan Tik Engine s.m.w.r.o";
...
onof.MouseButton1Click:Connect(function()
    ...
    if flying then
        ...
        -- Simple safe mode: put humanoid in PlatformStand so physics don't fight teleporting
        if humanoid then
...
-- Optional: keep a heartbeat loop to maintain simple "hover while flying"
local RunService = game:GetService("RunService")
local heartbeatConn
heartbeatConn = RunService.Heartbeat:Connect(function()
    if flying and hrp and hrp.Parent then
        -- keep player hovering at small upward offset to avoid falling between teleports
        local vel = Vector3.new(0, 0, 0)
        -- No continuous movement by default; up/down buttons and speed control are provided
        -- (If you want smoother WASD movement, we can add it here.)
        hrp.Velocity = vel
    end
end)

-- Clean up connection if GUI is destroyed
